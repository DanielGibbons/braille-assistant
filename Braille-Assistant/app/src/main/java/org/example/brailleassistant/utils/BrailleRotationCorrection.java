...
public class BrailleRotationCorrection {

    static public class BrailleRotationCorrectionOutput_t {

        public BrailleFilter.BrailleFilterOutput_t filterOutput;
        public double rotation;
        public Point rotationCentre;

        public BrailleRotationCorrectionOutput_t(BrailleFilter.BrailleFilterOutput_t filterOutput, double rotation, Point rotationCentre) {
    ...
    public BrailleRotationCorrection() {

    }

    // public API function
    public BrailleRotationCorrectionOutput_t correctImageRotation(BrailleFilter.BrailleFilterOutput_t filterOutput) {

        double rotation = this.findImageRotation(filterOutput.brailleDotLocations);
        if (rotation == 0 || rotation == NaN) {
            return new BrailleRotationCorrectionOutput_t(filterOutput, 0, new Point(0, 0));
        }
        Mat rotatedBrailleImage = rotateImage(filterOutput.brailleImage, rotation);
        double[][] rotatedImageDotLocations = findNewDotLocations(rotatedBrailleImage, filterOutput.brailleDotLocations);

        return new BrailleRotationCorrectionOutput_t(new BrailleFilter.BrailleFilterOutput_t(rotatedBrailleImage, rotatedImageDotLocations), rotation, new Point(filterOutput.brailleImage.width(), filterOutput.brailleImage.height()));
    }

    // finds new dot locations - these changed due to image rotation
    private double[][] findNewDotLocations(Mat filteredBrailleImage, double[][] dotLocations) {
